...
const en: Record<MessageKey, string> = {
  ...
  "rsm.gyroHint": "The mouse drives the DS4 gyroscope (motion aiming) — the stick stays centered.",
    ...
    "rsm.dsuHint": "Streams the gyroscope over UDP (port 26760) — the DS4's HID report never carries motion data, so this is the only motion source that ever reaches the game. In Ryujinx: Options → Settings → Input → Motion → 'Use CemuHook compatible motion' (server 127.0.0.1, port 26760). Close pad-motion before enabling. The log must show 'DSU client connected' — otherwise the emulator isn't reading the server.",
