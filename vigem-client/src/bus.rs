...
/// Extended DS4 report submission: the full 63-byte HID report (gyro, accel,
/// touchpad...). Same IOCTL as the basic report — the driver distinguishes
/// the two by the Size field ("Same IOCTL, just different size" in the C SDK).
///
/// The C struct is `pack(1)`; to avoid unaligned packed-field access in Rust
/// we serialize into a flat byte buffer instead:
/// `Size: u32 LE` (4) + `SerialNo: u32 LE` (4) + report (63) = 71 bytes.
#[cfg(feature = "unstable_ds4")]
...
impl DS4SubmitReportEx {
	#[inline]
	pub fn new(serial_no: u32, report: &crate::DS4ReportEx) -> DS4SubmitReportEx {
		let mut buffer = [0u8; 71];
		buffer[0..4].copy_from_slice(&71u32.to_le_bytes());
		buffer[4..8].copy_from_slice(&serial_no.to_le_bytes());
		report.write_to(&mut buffer[8..71]);
		DS4SubmitReportEx { buffer }
	}
	#[inline]
	pub unsafe fn ioctl(&mut self, device: HANDLE, event: HANDLE) -> Result<(), u32> {
		let mut transferred = 0;
